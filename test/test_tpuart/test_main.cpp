...
struct Fixture
{
    ...
    // Beantwortet, was die Library an die BCU schickt, als läge der Bus an - bis die Verbindung steht und
    // das Registerlesen durch ist. Die Vorgabewerte sind ein NCN5130.
    void answerBcu(uint8_t wd = NCN_WD_RESET,
                   uint8_t acr1 = NCN_ACR1_RESET_5121_5130,
                   uint8_t revId = NCN_PART_5130,
                   bool answerStop = true)
    {
        bool stopRequested = false;
        bool finished = false;
        int connectedFor = 0;
        size_t answered = _interface.writtenBytes().size();
        for (int i = 0; i < 3000 && !finished; i++)
        {
            const std::vector<uint8_t> &out = _interface.writtenBytes();
            while (answered < out.size())
            {
                uint8_t code = out[answered++];
                if (code == U_STOP_MODE_REQ)
                {
                    stopRequested = true;
                    if (answerStop) _interface.addByte((char)U_STOP_MODE_IND, 0);
                }
                else if (code == (uint8_t)(U_INT_REG_RD_REQ | NCN_REG_WD)) _interface.addByte((char)wd, 0);
                else if (code == (uint8_t)(U_INT_REG_RD_REQ | NCN_REG_ACR1)) _interface.addByte((char)acr1, 0);
                else if (code == (uint8_t)(U_INT_REG_RD_REQ | NCN_REG_REVID)) _interface.addByte((char)revId, 0);
                else if (code == U_RESET_REQ)
                {
            ...
            if (_dll.isConnected() && !stopRequested && ++connectedFor > 5) finished = true;
        }
    }

    // Verbindungsaufnahme samt Registerlesen durchspielen.
    void connect(BcuType type = BcuType::Ncn5120,
                 uint8_t wd = NCN_WD_RESET,
                 uint8_t acr1 = NCN_ACR1_RESET_5121_5130,
                 uint8_t revId = NCN_PART_5130,
                 bool answerStop = true)
    {
        _dll.begin(type);
        answerBcu(wd, acr1, revId, answerStop);
    ...
    // Verbindet als NCN und beantwortet dabei nur die Resets - bis die Library den Stop für den
    // Registerzugriff anfordert.
    void connectUntilStopRequested(bool answerStop)
    {
        _dll.begin(BcuType::Ncn5120);

        size_t answered = 0;
        for (int i = 0; i < 3000; i++)
        {
            const std::vector<uint8_t> &out = _interface.writtenBytes();
            for (; answered < out.size(); answered++)
            {
                if (out[answered] == U_RESET_REQ) _interface.addByte((char)U_RESET_IND, 0);
                if (out[answered] != U_STOP_MODE_REQ) continue;
...
// Die Zustände werden in fester Reihenfolge durchlaufen.
static void test_bcu_state_runs_through_all_states()
{
    TEST_ASSERT_EQUAL((int)BcuState::Uninitialized, (int)fx->_dll.bcuState());

    fx->_dll.begin(BcuType::Ncn5120);
    TEST_ASSERT_EQUAL((int)BcuState::Searching, (int)fx->_dll.bcuState());

    fx->connectUntilStopRequested(false);
    TEST_ASSERT_EQUAL((int)BcuState::Identifying, (int)fx->_dll.bcuState());
    TEST_ASSERT_FALSE(fx->_dll.startMonitoring());

    fx->pump(80);
    TEST_ASSERT_EQUAL((int)BcuState::Connected, (int)fx->_dll.bcuState());

    TEST_ASSERT_TRUE(fx->_dll.startMonitoring());
    fx->pump(5);
    TEST_ASSERT_EQUAL((int)BcuState::BusMonitor, (int)fx->_dll.bcuState());

    TEST_ASSERT_TRUE(fx->_dll.reset());
    fx->pump(5);
    TEST_ASSERT_EQUAL((int)BcuState::Connected, (int)fx->_dll.bcuState());

    fx->pump(TPUART_CONNECTION_TIMEOUT_MS + 100);
    TEST_ASSERT_EQUAL((int)BcuState::Disconnected, (int)fx->_dll.bcuState());

    fx->_dll.end();
    TEST_ASSERT_EQUAL((int)BcuState::Uninitialized, (int)fx->_dll.bcuState());
}

// Setzt die BCU sich selbst zurück, endet der Busmonitor auch ohne eigenes U_Reset.req.
static void test_reset_indication_ends_bus_monitor()
{
    fx->connect();
    TEST_ASSERT_TRUE(fx->_dll.startMonitoring());
    fx->pump(5);
...
{
    fx->connect();
    fx->_dll.getStatistics().reset(); // die Verbindungsaufnahme läuft selbst durch den Ring
...
// Liegt beim Start noch kein Bus an, sucht die Erkennung weiter und verbindet, sobald er da ist.
static void test_connect_when_bus_arrives_late()
{
    fx->_dll.begin(BcuType::Ncn5120);
    fx->pump(2500);
    TEST_ASSERT_FALSE(fx->_dll.isConnected());
    ...
    TEST_ASSERT_TRUE(fx->_dll.isConnected());
    TEST_ASSERT_EQUAL((int)BcuChip::Ncn5130, (int)fx->_dll.bcuChip());
}

// Bus ab und wieder an: erst Verlust, dann neue Verbindung mit der bekannten Baudrate.
static void test_reconnect_when_bus_returns()
{
    fx->connect();
    uint32_t baud = fx->_dll.connectedBaudRate();

    fx->pump(TPUART_CONNECTION_TIMEOUT_MS + 1500);
    TEST_ASSERT_FALSE(fx->_dll.isConnected());

    size_t before = fx->_interface.writtenBytes().size();
    fx->answerBcu();

    TEST_ASSERT_EQUAL((int)BcuState::Connected, (int)fx->_dll.bcuState());
    TEST_ASSERT_EQUAL(baud, fx->_dll.connectedBaudRate());

    // Der Chip ist schon bestimmt - ein Wiederverbinden fährt ihn nicht erneut in den Stop.
    const std::vector<uint8_t> &written = fx->_interface.writtenBytes();
    for (size_t i = before; i < written.size(); i++)
        TEST_ASSERT_NOT_EQUAL(U_STOP_MODE_REQ, written[i]);
}
...
// ACR0 geht auf einem NCN nach jedem Reset raus, ohne dass jemand powerControl() gerufen hat.
static void test_power_control_is_applied_by_default_on_ncn()
{
    fx->connect(BcuType::Ncn5120);
    ...
    TEST_ASSERT_TRUE(fx->written({(uint8_t)(U_INT_REG_WR_REQ | NCN_REG_ACR0), 0x74}));
}

// Auf dem 5120 ist ACR0 Bit 2 reserviert und bleibt ungesetzt.
static void test_power_control_leaves_reserved_bit_on_ncn5120()
{
    fx->connect(BcuType::Ncn5120, NCN_WD_RESET, NCN_ACR1_RESET_5120);

    TEST_ASSERT_TRUE(fx->_dll.powerControl(false));
    fx->pump(20);

    TEST_ASSERT_TRUE(fx->written({(uint8_t)(U_INT_REG_WR_REQ | NCN_REG_ACR0), 0x10}));
}

// Registerschreiben setzt die 2-Byte-Gruppe ab, Opcode aus 0x28 plus Registernummer.
static void test_write_register_sends_opcode_and_value()
{
    fx->connect(BcuType::Ncn5120);

    TEST_ASSERT_TRUE(fx->_dll.writeRegister(NCN_REG_ACR1, 0x60));
    fx->pump(20);

    TEST_ASSERT_TRUE(fx->written({(uint8_t)(U_INT_REG_WR_REQ | NCN_REG_ACR1), 0x60}));
}

// Auf einem TPUART2 wäre 0x28 U_SetAddress.req - es darf deshalb NICHTS rausgehen, nicht bloß false
// zurückkommen.
static void test_write_register_is_rejected_on_tpuart2()
{
    fx->connect(BcuType::Tpuart2);

    TEST_ASSERT_FALSE(fx->_dll.writeRegister(NCN_REG_ACR0, 0x54));
    fx->pump(20);

    TEST_ASSERT_EQUAL(0, fx->_interface.writtenBytes().size());
}

// Eine Registernummer über 2 Bit liefe in den Opcode-Bereich daneben - abgelehnt, ohne etwas zu senden.
static void test_write_register_rejects_out_of_range_number()
{
    fx->connect(BcuType::Ncn5120);

    TEST_ASSERT_FALSE(fx->_dll.writeRegister(U_INT_REG_WR_ADDRESS_MASK + 1, 0x00));
    fx->pump(20);
...
static void test_identify_reads_chip_type_and_revision()
{
    fx->connect(BcuType::Ncn5120, NCN_WD_RESET, NCN_ACR1_RESET_5121_5130, (uint8_t)(0x03 << NCN_REVID_REVISION_SHIFT | NCN_PART_5130));

    TEST_ASSERT_TRUE(fx->_dll.isConnected());
    TEST_ASSERT_EQUAL((int)BcuChip::Ncn5130, (int)fx->_dll.bcuChip());
    TEST_ASSERT_EQUAL(0x03, fx->_dll.ncnRevision());
}

static void test_identify_detects_ncn5120_without_reading_revid()
{
    fx->connect(BcuType::Ncn5120, NCN_WD_RESET, NCN_ACR1_RESET_5120, (uint8_t)(0x03 << NCN_REVID_REVISION_SHIFT | NCN_PART_5130));

    TEST_ASSERT_EQUAL((int)BcuChip::Ncn5120, (int)fx->_dll.bcuChip());
    TEST_ASSERT_EQUAL(0, fx->_dll.ncnRevision());
}

// Je begin() wird der Chip neu bestimmt; nach end() gilt er als unbekannt.
static void test_identify_runs_again_after_begin()
{
    fx->connect(BcuType::Ncn5120, NCN_WD_RESET, NCN_ACR1_RESET_5120);
    TEST_ASSERT_EQUAL((int)BcuChip::Ncn5120, (int)fx->_dll.bcuChip());

    fx->_dll.end();
    TEST_ASSERT_EQUAL((int)BcuChip::Unknown, (int)fx->_dll.bcuChip());

    fx->connect(BcuType::Ncn5120);
    TEST_ASSERT_EQUAL((int)BcuChip::Ncn5130, (int)fx->_dll.bcuChip());
}

// end() mitten im Registerzugriff: nach begin() darf das Warten auf den Wert nicht das erste Byte fressen.
static void test_identify_after_end_during_register_read()
{
    fx->connectUntilStopRequested(true);
    fx->pump(3);
    fx->_dll.end();
...
static void test_identify_stops_when_watchdog_anchor_mismatches()
{
    fx->connect(BcuType::Ncn5120, 0x42);

    TEST_ASSERT_TRUE(fx->_dll.isConnected());
    TEST_ASSERT_EQUAL((int)BcuChip::Unknown, (int)fx->_dll.bcuChip());
}

static void test_identify_timeout_does_not_block_connect()
{
    fx->connect(BcuType::Ncn5120, NCN_WD_RESET, NCN_ACR1_RESET_5121_5130, NCN_PART_5130, false);

    TEST_ASSERT_TRUE(fx->_dll.isConnected());
    TEST_ASSERT_EQUAL((int)BcuChip::Unknown, (int)fx->_dll.bcuChip());
}

static void test_identify_is_skipped_on_tpuart2()
{
    fx->connect(BcuType::Tpuart2, NCN_WD_RESET, NCN_ACR1_RESET_5121_5130, (uint8_t)(0x03 << NCN_REVID_REVISION_SHIFT | NCN_PART_5130));

    TEST_ASSERT_TRUE(fx->_dll.isConnected());
    TEST_ASSERT_EQUAL(0, fx->_dll.ncnRevision());
    TEST_ASSERT_EQUAL((int)BcuChip::Tpuart2, (int)fx->_dll.bcuChip());
}

static size_t indexOf(const std::vector<uint8_t> &data, uint8_t value, size_t from = 0)
{
...
// Solange der Registerzugriff läuft, halten Konfiguration und Telegramm - danach gehen beide raus.
static void test_register_read_holds_back_configuration_and_telegrams()
{
    fx->connectUntilStopRequested(false);
    fx->_interface.clearWrittenBytes();

    fx->_dll.setOwnAddress(0x1101);
    std::vector<uint8_t> frame = standardFrame();
    TEST_ASSERT_TRUE(fx->_dll.pushTransmitQueue(frame.data(), frame.size()));
    fx->pump(30);
    ...
    fx->pump(30);
    TEST_ASSERT_TRUE(indexOf(fx->_interface.writtenBytes(), U_RESET_REQ) < fx->_interface.writtenBytes().size());

    fx->_interface.addByte((char)U_RESET_IND, 0);
    fx->pump(100);

    const std::vector<uint8_t> &written = fx->_interface.writtenBytes();
    TEST_ASSERT_TRUE(indexOf(written, U_NCN5120_SET_ADDRESS_REQ) < written.size());
    TEST_ASSERT_EQUAL(1, fx->_dll.getStatistics().getTxFrames());
}

// Während des Registerzugriffs keine Statusabfrage - ihre Antwort landete sonst als Registerwert.
static void test_register_read_rejects_state_request()
{
    fx->connectUntilStopRequested(false);
    fx->_interface.clearWrittenBytes();
...
// Bleibt ein Registerwert aus, endet das Warten darauf mit dem Reset - das nächste Byte gehört wieder dem Bus.
static void test_missing_register_value_does_not_swallow_next_byte()
{
    fx->connectUntilStopRequested(true);
    fx->pump(40);

    const std::vector<uint8_t> &written = fx->_interface.writtenBytes();
    size_t read = indexOf(written, (uint8_t)(U_INT_REG_RD_REQ | NCN_REG_WD));
    TEST_ASSERT_TRUE(read < written.size());
    TEST_ASSERT_TRUE(indexOf(written, U_RESET_REQ, read) < written.size());
...
// Verkehr auf dem Bus verbraucht die Frist für den Stop nicht - sie läuft erst ab Busruhe.
static void test_stop_timeout_starts_after_bus_traffic()
{
    fx->connectUntilStopRequested(false);
    fx->_interface.clearWrittenBytes();
    ...
    fx->feed(stream);
    fx->pump(95);
    TEST_ASSERT_EQUAL(fx->_interface.writtenBytes().size(), indexOf(fx->_interface.writtenBytes(), U_RESET_REQ));

    fx->pump(80);
    TEST_ASSERT_TRUE(indexOf(fx->_interface.writtenBytes(), U_RESET_REQ) < fx->_interface.writtenBytes().size());
}

// Kommt der Stop nie, endet der Registerzugriff trotz Dauerverkehr - sonst stünden Senden und Überwachung.
static void test_stop_timeout_has_upper_bound_under_permanent_traffic()
{
    fx->connectUntilStopRequested(false);
    fx->_interface.clearWrittenBytes();
...
static int runAllTests()
{
    ...
    RUN_TEST(test_bcu_state_runs_through_all_states);
    RUN_TEST(test_reset_indication_ends_bus_monitor);
    ...
    RUN_TEST(test_connect_when_bus_arrives_late);
    RUN_TEST(test_reconnect_when_bus_returns);
    ...
    RUN_TEST(test_power_control_is_applied_by_default_on_ncn);
    RUN_TEST(test_power_control_leaves_reserved_bit_on_ncn5120);
    RUN_TEST(test_write_register_sends_opcode_and_value);
    RUN_TEST(test_write_register_is_rejected_on_tpuart2);
    RUN_TEST(test_write_register_rejects_out_of_range_number);
    RUN_TEST(test_identify_reads_chip_type_and_revision);
    RUN_TEST(test_identify_detects_ncn5120_without_reading_revid);
    RUN_TEST(test_identify_runs_again_after_begin);
    RUN_TEST(test_identify_after_end_during_register_read);
    RUN_TEST(test_identify_stops_when_watchdog_anchor_mismatches);
    RUN_TEST(test_identify_timeout_does_not_block_connect);
    RUN_TEST(test_identify_is_skipped_on_tpuart2);
    RUN_TEST(test_register_read_holds_back_configuration_and_telegrams);
    RUN_TEST(test_register_read_rejects_state_request);
    RUN_TEST(test_missing_register_value_does_not_swallow_next_byte);
    RUN_TEST(test_stop_timeout_starts_after_bus_traffic);
    RUN_TEST(test_stop_timeout_has_upper_bound_under_permanent_traffic);
